  ...
  Thread.new do
    ...
    loop do
      ...
      if video.nil?
        ...
        begin
          result = system("yt-dlp -f \"bv[height<=1080][vcodec~='^((he|a)vc|h26[45])']+ba/bv[height<=1080]+ba/bv+ba\" --recode-video \"mp4\" --audio-format \"aac\" -o \"/downloads/%(uploader)s/%(upload_date)s - %(title)s.%(ext)s\" \"#{video.url}\"")
          if result
