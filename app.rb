  ...
  Thread.new do
    ...
    loop do
      ...
      if video.nil?
        ...
        begin
          result = system("yt-dlp -f \"bv*[height<=1080][vcodec~='^((he|a)vc|h26[45])']+ba[ext=m4a]/b[height<=1080][vcodec~='^((he|a)vc|h26[45])']/bv*[ext=mp4]+ba[ext=m4a]/b[ext=mp4]/bv*+ba/b\" --recode-video \"mp4\" --audio-format \"aac\" -o \"/downloads/%(uploader)s/%(upload_date)s - %(title)s.%(ext)s\" \"#{video.url}\"")
          if result
