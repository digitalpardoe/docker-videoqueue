  ...
  Thread.new do
    ...
    loop do
      ...
      if video.nil?
        ...
        begin
          result = system("yt-dlp -f bestvideo[ext=mp4][vcodec!*=av01][vcodec!*=vp9]+bestaudio[ext=m4a]/best[ext=mp4]/best --recode-video mp4 --audio-format m4a -o '/downloads/%(uploader)s/%(title)s.%(ext)s' #{video.url}")
          if result
